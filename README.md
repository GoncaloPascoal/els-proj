...
- `files`: list of paths to files or RegEx expressions containing the data to extract.
- `metadataColumns` (optional): map of strings to strings, where keys denote a column name and values denote the type of file metadata to store in that column. Metadata columns always appear at the beginning of the table. Valid metadata types are:
  - `fileName`
  - `filePath`
  - `absoluteFilePath`
  - `directoryName`
  - `directoryPath`
- `columnSuffix` (optional): suffix to be added to the name of all extracted columns. When unspecified, it doesn't affect the columns' names.

#### Load Structured Instruction (`loadStructured`)

Extracts a table from a file or set of files with a tree-like structure.
Inherits the `load` instruction's parameters. \
Specific Parameters:
- `paths`: list of XPath strings corresponding to nodes containing the columns we wish to extract.
- `columns` (optional): list of strings containing the names of the columns to extract. When unspecified, all elements within the chosen section will be extracted.

Note that only columns containing primitive data will be extracted (columns containing lists or maps are excluded). \
Column specification for multiple paths is not supported. TODO: check this sentence

#### Load Unstructured Instruction (`loadUnstructured`)

Extracts a table from a file or set of unstructured files.
Inherits the `load` instruction's parameters. \
Specific Parameters:
- `textInstructions`: list of text instructions that specify how to extract the table.
  ...
  - `columnInterval`:
    - `lines`: file lines from which to extract the columns. These can be specified as single lines (`12`) or line intervals (`15-20`). The first line of a file is line 1.
    - `columnIntervals`: mapping of column names to their position in the extracted line. For example, the interval `5-30` specifies that the value for that column spans file columns 5 through 30. When the interval is specified as a single value, for example, `50`, the interval is considered to start at that column, in this case 50, and end at the end of the line. The first column of a line is column 1.
    - `stripWhitespace` (optional): whether or not the whitespace surrounding the extracted information should be removed. Defaults to `true`.
    - `columnarFormat` (optional): TODO
  - `regexLineDelimiter`:
    - `linePatterns`: RegEx patterns corresponding to the beginning of the lines containing the data to extract.
    - `delimiter`: RegEx pattern representing the separation between the name and the value of the column.
...
Saves table as a CSV file, optionally filtering and reordering the columns to save. \
Parameters:
- `source`: identifier of the table to save.
- `file`: path to the file where the table will be saved. The file and its parent directories will be created if they do not exist.
- `columns` (optional): list containing the names of columns to save, in the desired order.
...
Parameters:
- `source`: identifier of the table whose columns we wish to rename.
- `mapping`: map of strings to strings, where the keys denote the old column names and the values denote the new column names. Note that an error will occur when trying to use an already existing column name (unless it is the same column).

#### Join Instruction (`join`)

Merges two or more tables, storing the result in a specified table. \
Parameters: 
- `tables`: identifiers of the tables to join.
- `target` (optional): identifier of the table where the result will be stored. Can correspond to a table that does not exist, in which case the table will be created. If omitted, will correspond to the first identifier in the `tables` list.
- `type`: type of join. TODO
  - `merge`: column-wise join
  - `concatenate`: row-wise join

#### Function Instruction

Applies a function to each of the columns of a table.
Cannot be used on its own. \
Parameters:
- `source`: identifier of the table whose columns we wish to apply the function to.
- `columns` (optional): list containing the names of columns where the function should be applied. When unspecified, the function will be applied to all the columns (except the ones specified in `excludeColumns`).
- `excludeColumns` (optional): list containing the names of the columns where the function should not be applied. When unspecified, no columns are excluded from the application of the function.
- `target` (optional): identifier of the table where the result will be stored. When unspecified, it is the source table. Note that the row generated by this instruction will affect subsequent operations (TODO: further function or sort instructions) in the table.

If the name of a column is specified both in `columns` and `excludeColumns`, the function will not be applied to that column.

#### Average Instruction (`average`)

Calculates the average of the values in each numeric column of the table.
Inherits the `function` instruction's parameters.

#### Sum Instruction (`sum`)

Calculates the sum of the values in each numeric column of the table.
Inherits the `function` instruction's parameters.

#### Sort Instruction (`sort`)

Parameters:
- `source`: identifier of the table which contains the column we want to sort.
- `colName`: name of the column to sort. 
- `descending` (optional): whether or not the column is sorted in descending order. Defaults to `false`.
